...
def create(unitId: int):
    ...
    if request.method == "POST":
        ...
        if error is None:
            try:
                db.execute(
                    "INSERT INTO model (unit, name, quantity, toughness, save, health, invuln, isLeader, enabled)"
                    "VALUES (?,?,?,?,?,?,?,?,?)",
                    (
                        unitId,
                        name,
...
def update(unitId: int, modelId: int):
    ...
    if request.method == "POST":
        ...
        if error is None:
            try:
                db.execute(
                    "UPDATE model SET name=?, quantity=?, toughness=?, save=?, health=?, invuln=?, isLeader=?, enabled=?"
                    "WHERE id=?"
                    "VALUES (?,?,?,?,?,?,?,?,?)",
                    (
                        name,
