...
from flaskr.db import get_db, get_unit, get_model
...
def create(unit_id):
    if request.method == "POST":
        ...
        db.execute(
            'INSERT INTO unit_models (unit_id, model_id, quantity)'
            ' VALUES ?,?,?',
            (unit_id, new_id, 1)
        )
        db.commit()
...
def modify(unit_id, model_id):
    ...
    if request.method == "POST":
        ...
        db.execute(
            ...
            ' WHERE id=?',
            (request.form['name'], request.form['movement'], request.form['toughness'], request.form['save'], 7, request.form['health'], model_id)
        )
    ...
    return render_template("model/modify.html", unit_id=unit_id, model=model, model_weapons=weapons)


@bp.route("/<int:model_id>/delete", methods=("DELETE"))
